...
<style>
  .grid-container {
    ...
    gap: 20px; /* Increase the gap to create more spacing between columns */
    padding: 10px;
  }
   ...
   <div style="max-width:1100px; margin:0 auto">
    <div style="background-color: #133764; color:white">
      <div class="w3-container">
        <h3><strong>Cemetery Grid Map</strong></h3>
      </div>
    </div>
    <br>
...
<form method="POST" action="ManageMap.php">
...
<div class="grid-container">
  ...
  $rows_per_page = 10;
  $columns_per_row = 10;
  $seats_per_page = $rows_per_page * $columns_per_row;
  $page_number = isset($_GET['pageno']) ? $_GET['pageno'] : 1;
  $start_index = ($page_number - 1) * $seats_per_page;
  $end_index = $start_index + $seats_per_page;
  $paged_rows = array_slice($rows, $start_index, $seats_per_page);

  // Calculate the number of rows and columns for the current page
  $rows_on_page = ceil(count($paged_rows) / $columns_per_row);
  $columns_on_page = min(count($paged_rows), $columns_per_row);

  // Iterate over each row
  for ($row = 0; $row < $rows_on_page; $row++) {
    echo '<div class="grid-row">';
    
    // Iterate over each column
    for ($col = 0; $col < $columns_on_page; $col++) {
      $index = $row * $columns_per_row + $col;
      if ($index < count($paged_rows)) {
        $cemetery = $paged_rows[$index];
        $image_path = $cemetery['Image'];
        $cmid = $cemetery['Grave_ID'];
        $Name_Deceased = $cemetery['Name_Deceased'];
        $Years_of_Born = $cemetery['Years_of_Born'];
        $Years_of_Died = $cemetery['Years_of_Died'];
        $Location = $cemetery['Location'];
        $Section = $cemetery['Section'];
        $Years_Buried = $cemetery['Years_Buried'];

        echo "<div class='grid-item'>";
        echo "<header style='background-color: #133764; color: white'>";
        echo "<h5><b>{$Name_Deceased}</b></h5>";
       
        echo "</header>";
        echo "<a href='GraveDetails.php?cmid=$cmid' class='btn btn-primary' style='text-decoration: none;'>View Detail</a><hr>";
        echo "</div>";
      }
    }

    echo '</div>'; // Close the grid-row div
  }
  ?>
</div>
<br>
      ...
      <nav class="pagination">
        <ul class="pagination-list">
          <?php
          $total_pages = ceil(count($rows) / $seats_per_page);
          for ($page = 1; $page <= $total_pages; $page++) {
            echo '<li class="pagination-item';
            if ($page == $page_number) {
              echo ' active';
            }
            echo '"><a href="ManageMap.php?pageno=' . $page . '" aria-label="Page ' . $page . '">' . $page . '</a></li>';
          }
          ?>
        </ul>
      </nav>
...
gridItems.forEach((item) => {
  item.addEventListener('click', () => {
    // Perform an action when a grid item is clicked
